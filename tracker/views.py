...
def dash(request,country):
	world = global_cases()
	if country == 'World':
		Region = world_data(world)
	else:
...
def Country_data(request):
	...
	deaths = deaths_report()[deaths_report()['Country/Region'] == 'India'].iloc[:,-1].values[0]
	recovered = recovered_report()[recovered_report()['Country/Region'] == 'India'].iloc[:,-1].values[0]
